...
Goal - To calculate the number of startles before loom using low pass filter data normalized for number of unmasked frames
"""
...
def spikes_position_new(tr,frame_list): #uses filter_speed
    list1 = []
    for j in range(tr.number_of_individuals):
        list2 = [i for i, value in enumerate(filter_speed_low_pass(tr)[frame_list,j]) if value > 10]
...
for i in temperature:
    ...
    for j in group:
        ...
        for k in replication:
            ...
            frame_list = np.r_[looms[0]+1000:looms[1],looms[1]+1000:looms[2],looms[2]+1000:looms[3],looms[3]+1000:looms[4]]   
            replicate_loom_startles[k] = len(spikes_position_new(tr, frame_list))*41612/filter_speed_low_pass(tr)[frame_list].compressed().shape[0]
...
fn1 = out_dir + 'nonloom_startles_normalized.p'
pickle.dump(loom_startles, open(fn1, 'wb')) # 'wb' is for write binary

fn2 = out_dir + 'nonloom_startles_normalized_std.p'
