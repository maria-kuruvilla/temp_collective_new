"""
Goal - to write one csv file with spontaneous startle in the preloom period using both speed and acc masks
Tue, July 27th 2021
...
def prop_spont_startles(tr, loom,s1=30,s2=3340, t=10): # for pre loom period
    count = 0
    for j in range(tr.number_of_individuals):
        list2 = []
        list2 = [i for i, value in enumerate(filter_speed_low_pass(tr,s1,s2)[0:(loom[0]),j]) if value > t]
...
def spont_startles(tr, loom, roi1 = 30, roi2 = 3340, t = 10): #uses filtered speed # n starts from 0 #for pre loom period
    list1 = spikes_position_new(tr,roi1 , roi2 , t)
    
    list2 = [i for i, value in enumerate(list1[:]) if value < (loom[0])]
    return(len(list2))
...
with open('../../data/temp_collective/roi/spontaneous_startles_preloom.csv', mode='w') as stats_speed:
    ...
    writer.writerow([
        ...
        'Time_fish_in', 'Time_start_record','prop_startles','number_startles', 'frames'])
            ...
            for k in replication:
                ...
                for m in range(len(met.Temperature)):
                    if met.Temperature[m] == i and met.Groupsize[m] == j and met.Replicate[m] == (k+1): 
                        ...
                        a = filter_speed_low_pass(tr,30,3340)[0:(looms[0]),:]
                        writer.writerow([
