...
def prop_startles(tr, loom, t):
    count = 0
    for j in range(tr.number_of_individuals):
        list2 = []
        list2 = [i for i, value in enumerate(filter_speed_low_pass(tr)[(loom+500):(loom+700),j]) if value > t]
...
for i in temperature:
    ...
    for j in group:
        ...
        for k in replication:
            ...
            for l in range(5):  
                startles_prop[l] = prop_startles(tr,looms[l],10)
...
fn1 = out_dir + 'prop_startles_no_nan_new_mask.p'
pickle.dump(average_prop_startles, open(fn1, 'wb')) # 'wb' is for write binary

fn2 = out_dir + 'prop_startles_no_nan_new_mask_std.p'
