"""
Goal - to write one csv file with spontaneous startle before loom and have loom as covariate and that have both speed and acc masks
Tue, July 13th 2021
...
def prop_spont_startles(tr, loom,n,s1=30,s2=3340, t=10):
    count = 0
    for j in range(tr.number_of_individuals):
        list2 = []
        list2 = [i for i, value in enumerate(filter_speed_low_pass(tr,s1,s2)[(loom[n]-200):(loom[n]),j]) if value > t]
...
def spont_startles(tr, loom, n, roi1 = 30, roi2 = 3340, t = 10): #uses filtered speed # n starts from 0
    list1 = spikes_position_new(tr,roi1 , roi2 , t)
    
    list2 = [i for i, value in enumerate(list1[:]) if value < (loom[n]) and value > (loom[n]-200) ]
...
with open('../../data/temp_collective/roi/spontaneous_startles_before_loom.csv', mode='w') as stats_speed:
    ...
    writer.writerow([
        ...
        'Time_fish_in', 'Time_start_record','Loom','prop_startles','number_startles'])
                    ...
                    if met.Temperature[m] == i and met.Groupsize[m] == j and met.Replicate[m] == (k+1): 
                        ...
                        for n in range(5):
                            ...
                            writer.writerow([
                                ...
                                prop_spont_startles(tr, looms,n),
                                spont_startles(tr,looms,n)])        
