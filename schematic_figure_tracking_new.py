"""
Goal - To produce figure with trajectories (different color for each fish) and convex hull area
Oct 21 2021
"""
...
n_frames = 900
hull_frame= 600
colors = plt.cm.viridis(np.linspace(0,1,tr.number_of_individuals ))
...
#for i in range(loom_no,loom_no+10):
	ax.scatter(filter_low_pass(tr)[0][loom_no:loom_no+n_frames,i], filter_low_pass(tr)[1][loom_no:loom_no+n_frames,i], color = colors[i], s = 10) #, color = colors[i+1]
hull = ConvexHull(tr.s[loom_no+n_frames]) 
hull2 = ConvexHull(tr.s[loom_no+hull_frame]) 
hull3 = ConvexHull(tr.s[loom_no])
for simplex in hull.simplices:

    plt.plot(tr.s[loom_no+n_frames][simplex, 0], tr.s[loom_no+n_frames][simplex, 1], color = 'black')
pts = tr.s[loom_no+n_frames][hull.vertices].reshape((-1,1,2))
ax.fill(pts[:,:,0],pts[:,:,1], facecolor = 'black', alpha = 0.2)
for simplex in hull2.simplices:

    plt.plot(tr.s[loom_no+hull_frame][simplex, 0], tr.s[loom_no+hull_frame][simplex, 1], color = 'black')
pts = tr.s[loom_no+hull_frame][hull2.vertices].reshape((-1,1,2))
ax.fill(pts[:,:,0],pts[:,:,1], facecolor = 'black', alpha = 0.2)
for simplex in hull3.simplices:

    plt.plot(tr.s[loom_no][simplex, 0], tr.s[loom_no][simplex, 1], color = 'black')
...
out_dir = '../../output/temp_collective/roi_figures/schematic_figure_4_no_background_new3.png'
