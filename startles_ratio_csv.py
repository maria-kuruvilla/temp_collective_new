...
Goal - number of startles accurate startles/ number of inaccurate startles (using only masked data). Write into csv.
"""
...
with open('../../data/temp_collective/roi/startles_ratio_zero.csv', mode='w') as stats_speed:
    writer = csv.writer(stats_speed, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)

    writer.writerow(['Temperature', 'Groupsize', 'Replicate', 'Trial','Date','Subtrial','Time_fish_in','Time_start_record','accuracy'])
            ...
            for k in replication:
                ...
                for l in range(1,len(rows1)):
                    if i == int(rows1[l][0]) and j == int(rows1[l][1]) and (k+1) == int(rows1[l][2]):
                        if  startles_list_new(tr) != 0 and np.isnan(accurate_startles(tr,i,j,k+1)) != True and accurate_startles(tr,i,j,k+1) != 0:
                            accuracy = accurate_startles(tr,i,j,k+1)*tr.number_of_individuals/startles_list_new(tr)
                            writer.writerow([i,j,k+1,int(rows1[l][3]),rows1[l][4],int(rows1[l][5]),rows1[l][6],rows1[l][7], accuracy])
