...
Goal - to produce a csv file with temp, gs rep, etc. and 90th, 50th, 99th percentile of non loom speed to do stats
...
with open('../../data/temp_collective/roi/stats_before_loom_percentile_speed_acc_low_mask.csv', mode='w') as stats_speed:
    writer = csv.writer(stats_speed, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)

    writer.writerow(['Temperature', 'Groupsize', 'Trial', 'Date', 'Subtrial','Time_fish_in', 'Time_start_record', '99_speed', '50_speed', '99_acc', '50_acc'])
    for i in temperature:
        ...
        for j in group:
            for k in replication:
                ...
                s_99 = np.percentile(filter_speed_low_pass(tr)[0:(int(looms[0])+500),:].compressed(),99)
                s_50 = np.percentile(filter_speed_low_pass(tr)[0:(int(looms[0])+500),:].compressed(),50)
                a_99 = np.percentile(filter_acc_low_pass(tr)[0:(int(looms[0])+500),:].compressed(),99)
                a_50 = np.percentile(filter_acc_low_pass(tr)[0:(int(looms[0])+500),:].compressed(),50)
                writer.writerow([i,j,met.Trial[m],met.Date[m],met.Subtrial[m],met.Time_fish_in[m],met.Time_start_record[m],s_99,s_50,a_99,a_50])