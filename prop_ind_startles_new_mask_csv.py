...
Goal - proportion of individuals that startle (using only new masked data)
"""
...
with open('../../data/temp_collective/roi/stats_prop_ind_startles_no_nan_new_mask.csv', mode='w') as stats_speed:
    writer = csv.writer(stats_speed, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)

    writer.writerow(['Temperature', 'Groupsize', 'Replicate', 'Trial', 'Date', 'Subtrial','Time_fish_in', 'Time_start_record','Loom','prop_startles10','prop_startles7','prop_startles10_40','prop_startles7_40'])
            ...
            for k in replication:
                ...
                for m in range(len(met.Temperature)):
                    if met.Temperature[m] == i and met.Groupsize[m] == j and met.Replicate[m] == (k+1): 
                        ...
                        for loom in range(5):
                            frame = looms[loom]
                            writer.writerow([i,j,k+1,met.Trial[m],met.Date[m],met.Subtrial[m],met.Time_fish_in[m],met.Time_start_record[m],loom+1,prop_startles(tr, frame, 10,30),prop_startles(tr, frame, 7,30),prop_startles(tr, frame, 10,40),prop_startles(tr, frame, 7,40)])        
